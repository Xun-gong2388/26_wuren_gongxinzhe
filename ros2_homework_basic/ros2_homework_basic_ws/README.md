...
```bash
# 进入工作区
cd ~/Desktop/ros2/ros2_homework/ros2_homework_basic/ros2_homework_basic_ws
...
## 三、作业完成思路
 ...
 让乌龟实现走一个8字环，其实本质上是画两个相切的圆而同时又要保证整个运动过程连续，所以应该在就相当于让乌龟在相切位置切换方向。那就是需要固定线速度和角速度，然后在相切位置，把原来绕z正方向的圆，变成绕z负方向的圆，从而实现8字环的轨迹。

## 四、完成作业遇到的困难及解决办法

### 困难 1：精确控制画圆的完整性

- 问题：如果运行时间不够精确，圆就画"过"或"不足"，导致两个圆的接点错位。
- 解决：用数学公式 T = 2π / ω 严格计算每圈的耗时，并使用累积时间计数器在每个定时器回调中精确累加 50ms，确保状态切换时机准确。

### 困难 2：配置编译环境
  
   问题：编写好代码后，直接运行就报错
   解决：利用ai工具，了解还需要下载什么依赖，以及运行代码的命令
...
编译代码过程中，AI协助我了解还需要下载什么依赖，以及运行代码的命令。

在完成一遍学习之后，利用ai给的实例代码帮助了解运行一个相机识别所需要的完整代码，从而理解ros2整体代码的结构是怎样的。

在编写setup.py文件时和package.xml文件时，由于还未掌握，所以大部分都是套用的ai的模板代码
